/**
 * Syntext MCP Server — server factory.
 *
 * Registers all tools and resources on an McpServer instance.
 * Kept separate from the stdio entry point (index.ts) so the server
 * can be exercised in tests via an in-memory transport.
 */
...
`
  ...
  // NOTE: SDK signature is resource(name, uri, readCallback) — the URI must be
  // the second argument.
  server.resource(
